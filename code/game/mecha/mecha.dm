...
/obj/mecha/Topic(href, href_list)
	...
	if(usr.stat > 0)
		return
	var/datum/topic_input/ti_filter = new /datum/topic_input(href,href_list)
	if(href_list["select_equip"])
		if(usr != src.occupant)	return
		var/obj/item/mecha_parts/mecha_equipment/equip = ti_filter.getObj("select_equip")
		if(equip)
			src.selected = equip
	...
	if(href_list["rfreq"])
		if(usr != src.occupant)	return
		var/new_frequency = (radio.frequency + ti_filter.getNum("rfreq"))
	...
	if(href_list["req_access"] && add_req_access)
		...
		output_access_dialog(ti_filter.getObj("id_card"),ti_filter.getMob("user"))
		return
	if(href_list["maint_access"] && maint_access)
		if(!in_range(src, usr))	return
		var/mob/user = ti_filter.getMob("user")
		if(user)
			if(state==0)
				state = 1
			...
			output_maintenance_dialog(ti_filter.getObj("id_card"),user)
		return
	...
	if(href_list["remove_passenger"] && state >= 1)
		...
		return
	if(href_list["add_req_access"] && add_req_access && ti_filter.getObj("id_card"))
		if(!in_range(src, usr))	return
		operation_req_access += ti_filter.getNum("add_req_access")
		output_access_dialog(ti_filter.getObj("id_card"),ti_filter.getMob("user"))
		return
	if(href_list["del_req_access"] && add_req_access && ti_filter.getObj("id_card"))
		if(!in_range(src, usr))	return
		operation_req_access -= ti_filter.getNum("del_req_access")
		output_access_dialog(ti_filter.getObj("id_card"),ti_filter.getMob("user"))
		return
