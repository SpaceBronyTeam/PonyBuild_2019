...
/mob/proc/changeling_power(var/required_chems=0, var/required_emotes, var/required_dna=0, var/max_genetic_damage=100, var/max_stat=0)
	...
	if(changeling.emote_charges < required_emotes)
		src << "<span class='warning'>We require at least [required_chems] units of chemicals to do that!</span>"
		return
...
/mob/proc/changeling_make_transform_cocoon(var/turf/location)
	set category = "Changeling"
	set name = "Make Transformation Cocoon (5)(5)"

	var/mob/living/carbon/pony/H = src
	if(!istype(H))
		src << "<span class='warning'>You may only use this power while in original form.</span>"
		return

	var/datum/changeling/changeling = changeling_power(5,1,1,0)
	if(!changeling)	return

	if(changeling.current_making)	return

	//создаем обьект в данной локации
	var/obj/structure/changeling_cocoon/transform/new_cocoon = new/obj/structure/changeling_cocoon/transform
	new_cocoon.Move(locate(location.x, location.y, location.z))
	changeling.current_making = new_cocoon

	for(var/t=1, t<=100, t++)
		sleep(1)
		if(new_cocoon in range(1))
			new_cocoon.change_progress(t)
		else
			sleep(100)
			if(new_cocoon in range(1))
				t--
