...
/obj/machinery/atmospherics/omni/filter/error_check()
	if(!input || !output || !atm_filters)
		return 1
	if(atm_filters.len < 1) //requires at least 1 filter ~otherwise why are you using a filter?
		return 1
...
/obj/machinery/atmospherics/omni/filter/process()
	...
	if (power_draw >= 0)
		...
		for(var/datum/omni_port/P in atm_filters)
			if(P.network)
				P.network.update = 1
