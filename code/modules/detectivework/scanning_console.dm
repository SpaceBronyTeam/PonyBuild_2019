...
/obj/machinery/computer/forensic_scanning/proc/get_filtered_set()
	...
	for(var/id in files)
		...
		var/add = 1
		if(scan_filters["name"])
			add = 0
			for(var/filter in scan_filters["name"])
				if(findtext(cur.fields["name"], filter))
					add = 1
					break

		if(scan_filters["area"])
			add = 0
			for(var/filter in scan_filters["area"])
				if(findtext(cur.fields["area"], filter))
					add = 1
					break

		if(scan_filters["fprints"])
			add = 0
			var/list/prints = cur.fields["fprints"]
			for(var/pid in prints)
				var/print = prints[pid]
				if (is_complete_print(print))
					for(var/filter in scan_filters["fprints"])
						if(findtext(print, filter))
							add = 1
							break

		if(scan_filters["fibers"])
			add = 0
			for(var/fiber in cur.fields["fibers"])
				for(var/filter in scan_filters["fibers"])
					if(findtext(fiber, filter))
						add = 1
						break

		if(scan_filters["blood"])
			add = 0
			for(var/DNA in cur.fields["blood"])
				for(var/filter in scan_filters["blood"])
					if(findtext(DNA, filter))
						add = 1
						break

		if(scan_filters["label"])
			add = 0
			for(var/filter in scan_filters["label"])
		...
		switch(screen)
			if("database")	//Database screen
				...
				for(var/filter in filternames)
					...
					dat += "<br>[filter]: <a href='?src=\ref[src];operation=filter;filter=[fname]'>[scan_filters[fname] ? list2text(scan_filters[fname], ",") : "All"]</a>"
...
/obj/machinery/computer/forensic_scanning/Topic(href,href_list)
	...
	switch(href_list["operation"])
		...
		if("filter")
			...
			if(filterstr)
				scan_filters[href_list["filter"]] = text2list(filterstr,",")
			else
				scan_filters[href_list["filter"]] = null
		if("screen")
