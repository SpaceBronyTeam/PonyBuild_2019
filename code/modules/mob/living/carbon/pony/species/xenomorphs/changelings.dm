...
/obj/structure/changeling_cocoon
	...
	var/full_icon_map[32][32]
	var/SoP = 0 //summ of pixels
	var/original_istate
	var/progress = 0
	...
	New()
		...
		for(var/px=1, px<=32, px++) for(var/py=1, py<=32, py++)
			var/clr = I.GetPixel(px, py, icon_state)
			if(clr != "#00000000" || clr)
				SoP++
				full_icon_map[x][y] = clr
		original_istate = icon_state
		icon_state = "blank"

	proc/change_progress(var/procent)
		if(procent<1 || procent>100)
			return

		progress = procent
		var/icon/I = src.icon
		//Дальше вычисляем высоту и расположение в строке
		for(var/px=1+(procent-1)*10, px<=px+10, px++)
			var/pix_y = round(px/32)
			var/pix_x = px-pix_y*32
			I.DrawBox(full_icon_map[pix_x][pix_y], pix_x, pix_y)
